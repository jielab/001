...
# 💇‍ binary 变量数据挖掘示例
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
dat <- dat0 %>% filter(ethnic.c=="White", sex==1) %>% mutate(
	bald.ord = factor(bald, levels=1:4, labels = c("ctrl", "bald2", "bald3", "bald4"), ordered=TRUE),
	bald.cat = factor(bald, levels=1:4, labels = c("ctrl", "bald2", "bald3", "bald4")),
	...
	stroke.y = ifelse(icdCt_stroke >1, 1, icdCt_stroke),
	stroke.c = ifelse(icdCt_stroke_h>=1 & icdCt_stroke_i==0, "hemo", ifelse(icdCt_stroke_i>=1 & icdCt_stroke_h==0, "ischemic", ifelse(icdCt_stroke_i==0 & icdCt_stroke_h==0, "ctrl", NA))),
)
