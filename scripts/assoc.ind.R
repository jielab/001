...
for (Y in Ys) { # 🙍
	...
	for (X in Xs) { # 🍷
		...
		dplyr::select(Y_yes, follow_years, X, age, sex, bmi, height, smoke_status, alcohol_status, PC1, PC2, grep("^bb_",names(dat1),value=T)) %>%
	...
	##	众多因素 vivid 交互作用分析 💃
		...
		#		labs(x="X labels", y="10-year risk (%)", title="") + scale_fill_manual(values=c("green", "gray", "orange"), name="Legend name") + theme_minimal() 
			...
			fit.M2Y <- survreg(Surv(follow_years, Y_yes) ~ Z + X +age+sex+PC1+PC2, data=dat1) # 🐕 这儿用surveg
