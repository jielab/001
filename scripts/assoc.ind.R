...
#	writeLines(paste('\n\n--> Run:', Y))
	...
	##	一般的 survival 分析和非线性 🏂 [rcssci_cox, rcssci_logistic, rcssci_linear]
		if (run_survival == "YES") {
			surv.obj <- Surv(time=dat1$follow_years, event=dat1$Y_yes)
			fit.cox <- coxph(surv.obj ~ X +age+sex, data=dat1); print(coef(summary(fit.cox)))
			...
			survminer::ggsurvplot(km.obj, ylim=c(0,0.2), fun="event", pval=TRUE, risk.table=TRUE, ggtheme = theme_bw(), palette=c("green","gray","orange"))
			survminer::ggforest(fit.cox, main="", cpositions=c(0, 0.1, 0.3), fontsize=1.2, data=dat1) # 不能显示interaction值
			rcssci_cox(data=dat1, time="follow_years", y="Y_yes", x="X", covs=c("age","sex"), prob=0.1, filepath='D:/analysis') # 🏮
		}
