...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
summary(lm(X ~ G1, dat1)); summary(lm(X ~ G2, dat1)) # G1强，G2弱
summary(lm(M ~ G1, dat1)); summary(lm(M ~ G2, dat1)) # G2和M没有显著性了
summary(lm(Y ~ G1, dat1)); summary(lm(Y ~ G2, dat1)) # G1和Y也快没有显著性了
dat1$X_byG1 = predict.lm( lm( X ~ G1, data=dat1)) 
dat1$X_byG2 = predict.lm( lm( X ~ G2, data=dat1)) 
summary(lm(Y ~ X, dat1));  summary(lm(Y ~ X_byG1, dat1)) # 对于一个很强的G1，G1基本就可代表“X本身”
summary(lm(Y ~ X, dat1));  summary(lm(Y ~ X_byG2, dat1)) # 对于一个很弱的G2，“G2所决定的XX” 跟 ”X本身“，差别很大
	beta_G2X = summary(lm( X ~ G1, data=dat1))$coef[2,1]
	beta_G2Y = summary(lm( Y ~ G1, data=dat1))$coef[2,1]
	se_G2X = summary(lm( X ~ G1, data=dat1))$coef[2,2]
	se_G2Y = summary(lm( Y ~ G1, data=dat1))$coef[2,2]
