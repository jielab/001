...
for (Y in Ys) { # 🙍
	...
	dat1 <- dat %>% mutate(
		Y_date=dat[[Y]],
		Y_yes=ifelse( is.na(Y_date), 0,1), Y_f=as.factor(Y_yes),
	...
	) %>% filter( follow_years >0 ) %>% slice_sample(n=10000) # 临时 🔔
	...
	for (X in Xs) { # 🍷
		...
		print(paste("RUN", X, Y))
		dat1$X=dat1[[X]]
		dat1$X[dat1$X=="slow"] <- NA; dat1$X=droplevels(dat1$X); table(dat1$X) # 对walk_pace 🔔
		dat1 <- dat1 %>% mutate(
		...
		) %>% drop_na(Y_f, X, age, sex, bmi, PC1, PC2)
		...
		fit.cox <- coxph(surv.obj ~ X +age+sex+bmi+smoke_status+alcohol_status+PC1+PC2, data=dat1); print(coef(summary(fit.cox)))
		fit.rforest <- randomForest(Y_f ~ X+ age+sex+bmi+PC1+PC2, na.action=na.omit, data=dat1)
		#fit.rforest <- rfsrc(surv.obj ~ ., data=dat1)
		fit.vivi <- vivi(fit=fit.rforest, response="Y_f", data=dat1); print(fit.vivi, digits=1)
		pdf(paste(Y,X,'.heatmap.pdf',sep='.')); viviHeatmap(mat=fit.vivi); dev.off()
		pdf(paste(Y,X,'.network.pdf',sep='.')); viviNetwork(mat=fit.vivi); dev.off()

		next # 🛑
		for (Z in Ms) { # ⛵ 
			# logfile=paste(X,Y,Z'.log',sep='.'); sink(logfile) # file.create(logfile)
			dat1$Z=dat1[[Z]]
			dat1 <- dat1 %>% mutate(
			...
			## 下面进行 mediation 分析 🐎🐎
			fit.X2M <- lm(Z ~ X, data=dat1) 
			fit.M2Y <- survreg(Surv(follow_years, Y_yes) ~ Z + X, data=dat1) # 🐕 这儿用surveg
