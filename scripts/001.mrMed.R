...
# 数据二合一
Y = 'covid_C2'; dir.Y = 'D:/data/gwas/main/clean' # 🙍
...
X = 'ABO'; dir.X = dir.Y; # 🍷
	...
	dat.Y.4x <- dat.Y.raw %>% merge(dat.X.iv, by="SNP") %>% format_data(type='outcome', snp_col='SNP', effect_allele_col='EA', other_allele_col='NEA', beta_col='BETA', se_col='SE', pval_col='P') %>% mutate(id.outcome =Y)		

# 数据三合一
M = 'bb_ALP'; dir.M = 'D:/data/gwas/bb/clean'; # 🐎
...
exit
# 简化版本，用于核对 # 👀
dat.XY <- harmonise_data(dat.X, dat.Y.4x, action=1) # 基于17个SNP，Total effect # ⚪
# dat.XY <- subset(dat, select=!grepl("mediator", names(dat)))
	fit.X2Y <- mr(dat.XY, method_list=c("mr_wald_ratio", "mr_ivw")); fit.X2Y 
	beta.X2Y <- fit.X2Y$b; se.X2Y <- fit.X2Y$se; p.X2Y <- rp(fit.X2Y$pval)
dat.XM <- harmonise_data(dat.X, dat.M.4x, action=1) # 基于14个SNP，Step1: X --> M # 🔔
#dat.XM <- subset(dat, select=!grepl("outcome", names(dat))); names(dat.XM) <- gsub("mediator", "outcome", names(dat.XM))
	fit.X2M <- mr(dat.XM, method_list=c("mr_wald_ratio", "mr_ivw")); fit.X2M 
	beta.X2M <- fit.X2M$b; se.X2M <- fit.X2M$se; p.X2M <- rp(fit.X2M$pval)
#dat.M2Y <- mr_mvinput(bx=cbind(dat$beta.mediator, dat$beta.exposure), bxse=cbind(dat$se.mediator, dat$se.exposure), by=dat$beta.outcome, byse=dat$se.outcome) # Step2: M (+X) --> Y # 🔔🔔
