...
for exp in $dats_x; do
	...
	if [ ! -e $exp_iv_file ]; then
		exp_iv_file=$exp.top.snp; zcat $exp_file0 | awk 'NR==1 || $10<=5e-6 {b=sprintf("%.0f",$3/1e6); print $4,$2,$3,$10,b}' | sort -k 2,2n -k 5,5n -k 4,4g | awk '{if (arr[$NF] !="Y") print $1; arr[$NF] ="Y"}' > $exp.top.snp
	fi
