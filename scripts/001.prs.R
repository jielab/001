...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
set.seed(12345)
trait='height'; binary='N' # trait='t2dm'; binary='Y'
...
dat <- dat0 %>% subset(select=grepl("^eid|^age|^sex$|^ethnic|^PC|^umap|t2dm|height", names(dat0))) %>% 
	filter(!is.na(PC1), !is.na(umap1), ethnic_cat %in% c("White", "Black", "Chinese", "Asian")) %>%
...
if (binary=="N") dat$trait=dat[[trait]]
if (binary=="Y") dat$trait=ifelse(is.na(dat[[paste0('icdDate_',trait)]]), 0, 1)
for (r in races){
...
dat$PC1 <- residuals(lm(PC1 ~ AFR.prs + EAS.prs + EUR.prs + SAS.prs +age+sex, na.action=na.exclude, data=dat ))
dat$PC2 <- residuals(lm(PC2 ~ AFR.prs + EAS.prs + EUR.prs + SAS.prs +age+sex, na.action=na.exclude, data=dat ))
dat <- dat %>% drop_na(race, trait, EUR.prs) %>% group_by(race) %>% slice_sample(n=1000) 
for (r in races){
	dat_sub <- subset(dat, race==r); print(paste(r,nrow(dat_sub)))
	dat_sub$prs.sameRace <- std(dat_sub[[paste0(r,".prs")]])
	...
	if (binary=="N") {
		model.train <- lm(trait ~ AFR.prs + EAS.prs + EUR.prs + SAS.prs +age+sex, data=dat_sub.train)
		y.hat <- predict(model.train, dat_sub.valid)
		print((cor(y.hat, dat_sub.valid$trait, use="complete.obs"))^2)
	}
	if (binary=="Y") {
		model.train <- glm(trait ~ AFR.prs + EAS.prs + EUR.prs + SAS.prs +age+sex, data=dat_sub.train)
		y.hat <- predict(model.train, dat_sub.valid)
		dfroc <- roc(dat_sub.valid$trait, y.hat)
...
for (r in races){
	...
	dat_sub.valid <- dat_sub[-ii,]
	if (binary=="N") {
		model.train <- lm(trait ~ AFR.prs_adj + EAS.prs_adj + EUR.prs_adj + SAS.prs_adj +age+sex, data=dat_sub.train)
		y.hat <- predict(model.train, dat_sub.valid)
		print((cor(y.hat, dat_sub.valid$trait, use="complete.obs"))^2)
	}
	if (binary=="Y") {
		model.train <- glm(trait ~ AFR.prs_adj + EAS.prs_adj + EUR.prs_adj + SAS.prs_adj +age+sex, data=dat_sub.train)
		y.hat <- predict(model.train, dat_sub.valid)
		dfroc <- roc(dat_sub.valid$trait, y.hat)
		print(plot(dfroc,col="red", legacy.axes=TRUE, print.auc=TRUE, print.thres=TRUE, grid=c(0.2,0.2),grid.col=c("blue","yellow")))
		print(auc(dfroc))
