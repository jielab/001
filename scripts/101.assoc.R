...
dat <- dat0 %>% drop_na(age, sex) %>%
	filter(ethnic_cat=="White") # %>% mutate( across(grep("walking", names(dat0), value=T), ~factor(.x))) # 批量变成 factor
Xs <- grep("score_sum$", names(dat), value=TRUE)
...
for (Y in Ys) {
	...
	for (X in Xs) {
		...
		print(ggsurvplot(survfit(surv.obj ~ X_qt, data=dat1), ylim=c(0,1), risk.table=FALSE))	
		dev.off()
