...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Y <- "icdDate_t2dm"
dat <- dat0 %>% filter(ethnicity_gen==1 & !is.na(o) & !is.na(se)) 
dat <- dat %>% 
	mutate(
...
surv.obj <- Surv(time=dat$follow_years, event=dat$Y_yes)
	ggsurvplot(survfit(surv.obj ~ o_se, data=dat), title=Y, risk.table=T, pval=T, ylim=c(0.9,1), font.main=c(16,"bold","darkblue"), font.x=c(14,"bold.italic","red"),font.y=c(14,"bold.italic","darkred"))
X <- "bb_GLU"; dat$X <- dat[[X]] # 做 cox 回归分析
fit.cox <- coxph(surv.obj ~ X+o_se +X*o_se +age+sex+PC1+PC2, data=dat); summary(fit.cox); coef(summary(fit.cox))
...
Xs <- "walkingpace"
Ys <- "icdDate_lungcancer"  #grep("^icdDate", names(dat), value=T)
Zs <- grep(".rs", names(dat), value=T)
