...
allele.qc <- function( # 🏮
  a1,a2,ref1,ref2) {
	...
	flip2=ref2; flip2[ref2=="A"]="T"; flip2[ref2=="T"]="A"; flip2[ref2=="G"]="C"; flip2[ref2=="C"]="G"
	snp=list()
...
IV.filter <- function( # 🏮
  dat, CHR_col, POS_col, AF_col, BETA_col, N_col) {
	filter1 <- dat[[CHR_col]]==6 & dat[[POS_col]] >= 25500000 & dat[[POS_col]] <= 34000000
...
run_mr2s <- function( # 🏮 常规的 TwoSampleMR
  label, X, dir.X, dat.X.raw, Y, dir.Y, dat.Y.raw) { 
	...
	write(paste(X,Y, nrow(dat.XY), rb(beta.X2Y), rb(se.X2Y), rp(p.X2Y), rp(p.hetero[1]), rp(p.hetero[2]), rp(p.pleio), rp(min(dat.XY$pval.outcome))), file=log_file, append=TRUE)
}
...
run_cisMr <- function( # 🏮 参考 https://github.com/ZhaotongL/cisMR-paper/blob/main/process_ARIC.R
  label, chr, flank, pos0, pos1, X, dat.X.raw, dir.X.cojo, Y, dat.Y.raw) {
	...
	bfile <- paste("--bfile", paste0(dir.X.cojo, "/", X), "--chr", chr) 
	...
	b_exp_cond=dat.mr$exp_df$bJ; b_out_cond=dat.mr$out_df$bJ 
	...
	Sig_exp_inv=solve(Sig_exp1); Sig_out_inv=solve(Sig_out1)
	res = cismr_cML_DP(
	...
	write(paste(X, Y, length(IV), rb(res$BIC_DP_theta), rb(res$BIC_DP_se), rp(res$BIC_DP_p)), file=log_file, append=TRUE)

}


run_coloc <- function( # 🏮
  label, chr, flank, pos0, pos1, X, dat.X.raw, Y, dat.Y.raw) {
	...
	write(paste(X, Y, nrow(dat.XY), res), file=log_coloc, append=TRUE)
