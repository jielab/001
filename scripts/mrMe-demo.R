...
pacman::p_load(data.table, stringi, dplyr, tidyverse, TwoSampleMR, MendelianRandomization, mrMed, RadialMR, psych, bda)
...
fit.M2Y <- lm(Y ~ M+X, data=dat1); summary(fit.M2Y) # 🐖 这儿必须捎带上X。 X的BETA=0.01667，是ADE，不再显著，表明 complete mediation。
	...
	beta.X2Y.dr <- coef.M2Y[3,1]; se.X2Y.dr <- coef.M2Y[3,2]; signif(coef.M2Y[3,4],2)
res <- mediation::mediate(fit.X2M, fit.M2Y, treat="X", mediator="M", boot=T); print(SUM <- summary(res))
	c(round(SUM$d.avg,3), round(SUM$z.avg,3), round(SUM$tau.coef,3), round(SUM$n.avg,3)) # ACME, ADE, Total, Prop
	beta.me <- beta.X2M * beta.M2Y; beta.me # 乘法
		se.me <- sqrt(beta.X2M^2 * se.X2M^2 + beta.M2Y^2 * se.M2Y^2); se.me # 🐕 POE法，也称Delta法 
		p.me <- signif(2*pnorm(-abs(beta.me/se.me)),2); p.me
	beta.dr <- beta.X2Y - beta.me; beta.dr # 减法
		se.dr <- sqrt(se.X2Y^2 + se.me^2); se.dr
		p.dr <- signif(2*pnorm(-abs(beta.dr/se.dr)),2); p.dr
