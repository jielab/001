...
run_mr2s <- function( # 🏮 常规的 TwoSampleMR
  analysis, p_t, label, X, dir.X, dat.X.raw, IV.file, IV_filter, Y, dir.Y, dat.Y.raw) {
	...
	write(paste(analysis, p_t, X,Y, nrow(dat.XY), rb(beta.X2Y), rb(se.X2Y), rp(p.X2Y), rp(p.hetero[1]), rp(p.hetero[2]), rp(p.pleio), rp(min(dat.XY$pval.outcome))), file=log_file, append=TRUE)
	return(p.X2Y)
}
