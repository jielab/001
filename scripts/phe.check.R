...
pacman::p_load(data.table, lubridate, dplyr, tidyverse, ggplot2, scatterplot3d, survival, survminer)
...
cor(dat$fto, dat$bmi, use="complete.obs") # 一些简单的分组统计
	aggregate(bmi ~ fto, data=dat, mean)
...
fit.lm <- lm(bmi ~ + as.factor(fto) + age+sex, data=dat, na.action=na.exclude) # 一些简单的线性回归和预测
...
plot(dat$bmi ~ dat$fto) # 一些简单的图
	...
	dat1 <- subset(dat, select=c("bmi", "bmi.prs3")) %>% rename(prs=bmi.prs3) %>% na.omit()
...
surv.obj <- Surv(time=dat$follow_years, event=dat$outcome_yes); surv.obj[1:20]
fit.suv <- survival::survfit(surv.obj ~ exposure, data=dat); summary(fit.suv, data=dat, times=1)
	survminer::ggsurvplot(fit.suv, ylim=c(0.95,1), data=dat) 
	survdiff(surv.obj ~ gen_3p, data=dat) # log-rank test
fit.cox <- coxph(surv.obj ~ exposure + gen + age+sex, data=dat); summary(fit.cox) # quantify effect size
