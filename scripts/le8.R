...
# 不吃 p6144, ⚡不正常饮食p20085, 🥛type_milk, type_yogurt, 🍞 type_bread, type_baguette, type_bap, type_roll


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# LE8 中南大学版本（参考文献 PMID: 40285703)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 地中海饮食 🥢
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# LE8 北大--南科大版本 DASH（参考文献 PMID： 39265807）
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
le8 <- merge(phe, srd, by="eid") %>% mutate( # drug.cmd[6177] drug.cmd2[6153]
	drug.lipid = ifelse(rowSums2(across(starts_with("drug.cmd"), ~ . %like% "1")) > 0, 1, 0)
)
le8 <- le8 %>% mutate(
...
le8 <- le8 %>% merge(dash, by="eid") %>% select(eid, all_of(paste0(vars.le8, ".pts"))) %>% mutate(
