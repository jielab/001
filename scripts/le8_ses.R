...
# 抗氧化剂🐏
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
fields <- read.table(paste0(dir0, "/data/ukb/phe/common/ukb.ato.dat"), sep="\t", header=F)
dat0 <- read.delim(paste0(dir0, "/data/ukb/phe/rap/ato.tab.gz"), sep="\t", fill=TRUE, header=T)
dat <- dat0 %>% mutate(across(matches("i[0-4]$") & !matches("p100010_i|p100020_i"),  ~ ifelse(get(paste0("p100010_", sub(".*_(i[0-4])$", "\\1", cur_column()))) %in% c(0, 5, 15), NA, .)))
for (i in fields$V1) {
  col_name <- fields$V2[fields$V1 == i]
  dat[[col_name]] <- rowMeans(select(dat, starts_with(i)), na.rm=TRUE)
}
dat <- dat %>% mutate_all(~ ifelse(is.numeric(.) & is.nan(.), NA, .))
dat <- subset(dat, select=c("eid", fields$V2))
saveRDS(dat, paste0(indir,"/Rdata/ukb.ato.rds"))
