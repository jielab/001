...
dat <- phe0 %>% 
	mutate(
		...
		pa_score[rowSums(mat_pa, na.rm=T) > 0] <- 1,
		pa_score[rowSums(is.na(mat_pa)) == 3] <- NA
	)
	fruit_score[rowSums(cbind(fruit_fresh, fruit_dried/5), na.rm = T) >= 4] <- T
	veg_score[rowSums(cbind(veg_cooked/3, veg_salad/3), na.rm=T) >= 4] <- T # vegetable: per serving = cooked/raw vegetables - 3 heaped tablespoons
	fish_score = (fish_oily >= 3) | (fish >= 3)
	fish_score[fish_oily ==2 & fish == 2] <- T
	pmeat_score = meat_processed <= 2
	npmeat_mat = cbind(beef, lamb, pork)
	npmeat_score = rowSums(npmeat_mat, na.rm = T) <= 3
	npmeat_score[npmeat_mat[,1] >=3 | npmeat_mat[,2] >=3 | npmeat_mat[,3] >=3] <- F
	grains_mat = matrix(0, nrow(habit_baseline_df), 2)
	bread_idx = which(bread_type == 3); grains_mat[bread_idx, 1] <- bread[bread_idx]
	cereal_idx = which(cereal_type %in% c(1, 2, 3))
	grains_mat[cereal_idx, 2] = cereal[cereal_idx]
	grains_score = rowSums(grains_mat, na.rm = T) >= 3
	diet_mat = data.frame(fruit_score, veg_score, fish_score, pmeat_score, npmeat_score, grains_score)
	diet_score = rowSums(diet_mat) >= 4
	diet_score[rowSums(diet_mat,na.rm = T) >= 4] <- T
	diet_score[rowSums(is.na(diet_mat)) + rowSums(diet_mat,na.rm = T) < 4] <- F
chrono_score = ifelse(sleep_chronotype < 3, 1, 0) # Ref PMID:31848595
	duration_score = ifelse(sleep_duration <= 8 & sleep_duration >= 7, 1, 0)
	insomnia_score = ifelse(sleep_insomnia == 1, 1, 0)
	snoring_score = ifelse(sleep_snoring == 2, 1, 0)
	narcolepsy_score = ifelse(sleep_narcolepsy < 2, 1, 0)
	...
	sleep_score[rowSums(sleep_mat,na.rm = T) >= 4] <- T
	sleep_score[rowSums(is.na(sleep_mat)) + rowSums(sleep_mat,na.rm = T) < 4] <- F
lf_df = data.frame(Nosmoke = nosmoke_cnd, Activity = activity_cnd, Diet = diet_cnd, Noalcohol = noalcohol_cnd, Sleep = sleep_cnd, Nocannabis = cannabis_cnd)
	...
	lifescore2 = rep(2, nrow(lf_df)); lifescore2[rowSums(lf_df, na.rm = T) >= 4] <- 3
	lifescore2[rowSums(lf_df[,c(1:5)], na.rm = T) >= 3] <- 3
	lifescore2[rowSums(lf_df == 0, na.rm = T) >= 4] <- 1
	lifescore3 = rep(2, nrow(lf_df)); lifescore3[rowSums(lf_df, na.rm = T) >= 4] <- 3
	lifescore3[rowSums(lf_df[,c(1:5)], na.rm = T) >= 3] <- 3
	lifescore3[rowSums(lf_df == 0, na.rm = T) >= 5] <- 1
	life_factor_df = data.frame(eid "_score", lifescore1, lifescore2, lifescore2)
